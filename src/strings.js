...
ヒント: まずは  ls  、そのあと  cd projects/  を試してみてください。`,
    ...
    projects: [
      ...
      {
        desc: 'Raspberry Pi のスピーカー環境向けに、セルフホスト型の音楽プレイヤーを構築。YouTube 楽曲を検索してキューに追加する React 製フロントエンドと、リクエストを WebSocket 経由で Pi 側のデーモンに転送する Express/Prisma バックエンドを組み合わせています。Pi は yt-dlp で音声を取得し、mpv と ALSA を通じてそのままスピーカーへストリーミング。ストレージに余裕のない Pi 側に何も保存しなくて済む仕組みです。WebSocket 接続はアウトバウンドのみを開始するためファイアウォールのポート開放は不要で、全体を1つの Docker Compose ファイルで動かしています。',
      },
    ],
